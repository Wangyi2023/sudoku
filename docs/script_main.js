...
let game_field_information_collection = [];
let game_field_collection = [];
let main_board = [];
...
function start() {
    ...
    game_field_information_collection = config.GAME_FIELD_INFORMATION_COLLECTION;
    cell_size = config.CELL_SIZE;
    font_size = config.FONT_SIZE;
    game_field_size = config.GAME_FIELD_SIZE;
...
function init_main_board(target_empty_cells) {
    ...
    count_array = Array(game_field_size + 1).fill(0);

    for (const game_field_information of game_field_information_collection) {
        const game_field = new Game_Field({
            Main_Board: main_board,
            Size: game_field_information[0],
            Position: game_field_information[1],
        });
...
function count_solutions() {
    ...
    for (const game_field of game_field_collection) {
        ...
        counters.push(game_field.count_solutions(board_temp));
    }
...
function is_valid(row, col, num) {
    for (const game_field of game_field_collection) {
        if (!game_field.is_valid(main_board, row, col, num)) {
            return false;
        }
    }
    return true;
...
function find_next_uncompleted_number() {
    for (let next = current_number + 1; next <= game_field_size; next++) {
...
function find_next_number() {
    ...
    return current_number < game_field_size ? current_number + 1 : 1;
}
function find_previous_number() {
    if (current_number === 0) {
        return 0;
    }
    return current_number > 1 ? current_number - 1 : game_field_size;
}
function flash_conflicting_cells(row, col, num) {
    ...
    let conflicts = [];
    for (const game_field of game_field_collection) {
        conflicts = [...conflicts, ...game_field.find_conflict_cells(row, col, num)];
    }
